geocodificadorModulo = (function () {
  ...
  function usaDireccion (address, funcionALlamar) {
        /* función usaDireccion(dirección,funcionALlamar)
     ...
     geocodificador.geocode({'address': address}, function(results, status) {
          if (status === 'OK') {
            var latlng = results[0].geometry.location;
            funcionALlamar(address,latlng);
  ...
  function inicializar () {
    ...
    document.querySelector('#direccion').addEventListener('keypress', function (e) {
      ...
      if (key === 13) { // 13 is enter
        ...
        that.usaDireccion(direccion, direccionesModulo.agregarDireccionEnter)
      }
    })
  }
